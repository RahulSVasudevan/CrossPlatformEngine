...
Game::Game()
{
	...
	*(IndexData + 5) = 3;
	...
	Mat = new PS4Material(renderer, "/app0/Lamborginhi_Aventador_diffuse.gnf");
	dynamic_cast<PS4Material*>(Mat)->LoadTextures(GNF);
	...
	dynamic_cast<PS4Material*>(FloorMat)->LoadTextures(RAW);
	...
	dynamic_cast<PS4Material*>(planeMaterial)->LoadTextures(RAW);
